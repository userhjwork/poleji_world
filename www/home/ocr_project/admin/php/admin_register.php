...
try {
    ...
    // 아이디 중복 확인
    $stmt = $conn->prepare("SELECT id FROM ocr_project_admin WHERE username = ?");
    if (!$stmt) {
        ob_clean();
    ...
    // 관리자 등록
    $stmt = $conn->prepare("INSERT INTO ocr_project_admin (username, password) VALUES (?, ?)");
    if (!$stmt) {
        ob_clean();
        echo json_encode(['success' => false, 'message' => 'SQL 준비 오류: ' . $conn->error]);
        exit();
    ...
    if ($stmt->execute()) {
        ob_clean();
        echo json_encode(['success' => true, 'message' => '관리자 등록이 완료되었습니다.']);
    } else {
        ob_clean();
        echo json_encode(['success' => false, 'message' => '관리자 등록 중 오류가 발생했습니다: ' . $stmt->error]);
    }
