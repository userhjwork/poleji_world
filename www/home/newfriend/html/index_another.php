...
<head>
    ...
    <title>대구동부교회 바울 새가족부 지출내역 작성 (다부서)</title>
...
<script>
    ...
    $(function(){
        // 부서 선택 시 카테고리 데이터 가져오기
        $('#department_select').on('change', function() {
            const department = $(this).val();
            if (department) {
                $.getJSON('./php/get_dept_ctgry.php', { department: department }, function(data) {
                    ctgryData = data;
                    renderCtgry1();
                }).fail(function(err) {
        ...
        // 초기 카테고리 렌더링 (기본값: 기타)
        $.getJSON('./php/get_dept_ctgry.php', { department: '기타' }, function(data) {
            ctgryData = data;
            renderCtgry1();
        });
    });
    ...
    function renderCtgry1() {
        ...
        if (ctgryData.ctgry_1) {
            ctgryData.ctgry_1.forEach(c1 => {
    ...
    function renderCtgry2(ctgry_1) {
        let html = '<option value="">선택</option>';
        if (ctgryData.ctgry_2 && ctgryData.ctgry_2[ctgry_1]) {
            ctgryData.ctgry_2[ctgry_1].forEach(c2 => {
    ...
    function renderCtgry3(ctgry_1, ctgry_2) {
        let html = '<option value="">선택</option>';
        if (ctgryData.ctgry_3 && ctgryData.ctgry_3[ctgry_2]) {
            const desc = ctgryData.ctgry_3[ctgry_2][0] || '';
            html += `<option value="${ctgry_2}" title="${desc}">${ctgry_2}</option>`;
        }
    ...
    $("#btn_submit").click(function(){
        ...
        // === 전송 ===
        $.ajax({
            url: './php/insert_out_another.php',
            type: 'POST',
            data: {
