...
ORDER BY date ASC;";
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $list_html .= "    <button type=\"button\" class=\"list_cell_wrap\" o_idx=\"". htmlspecialchars($row['idx']) ."\">";
            $list_html .= "        <div class=\"list_cell\">";
            $list_html .= "            <div class=\"list_cell_title\">";
            $list_html .= "                <div class=\"date_wrap\">";
            $list_html .= "                    <span class=\"date o_pay data_cell\">" . htmlspecialchars($row['date']) . "</span>";
            ...
            $list_html .= "                    <span class=\"amount data_cell\">" . (is_numeric($row['amount']) ? number_format($row['amount']) : '-') . "</span>";
            $list_html .= "                    <span class=\"text\">원</span>";
            $list_html .= "                </div>";
            $list_html .= "                <div class=\"desc_wrap\">";
            $list_html .= "                    <span class=\"desc data_cell\">" . nl2br(htmlspecialchars($row['description'])) . "</span>";
            $list_html .= "                </div>";
            $list_html .= "            </div>";
            $list_html .= "        </div>";
            $list_html .= "    </button>";
