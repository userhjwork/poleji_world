...
$id = $_GET['id'] ?? null;
if (!$id) {
    header('Location: index_multi_history.php');
    exit;
}

// === 데이터 조회 ===
$sql = "SELECT * FROM tbl_church_out_multi WHERE id = :id";
...
<script>
    ...
    $(function(){
        $.getJSON('./php/get_ctgry_all.php', function (data) {
            ...
            // 기존 값 선택
            $('#department_select').val('<?= $record['department'] ?>');
            $('#chr_ctgry_1').val('<?= $record['ctgry_1'] ?>');
            renderCtgry2('<?= $record['ctgry_1'] ?>');
            $('#chr_ctgry_2').val('<?= $record['ctgry_2'] ?>');
            renderCtgry3('<?= $record['ctgry_1'] ?>', '<?= $record['ctgry_2'] ?>');
            $('#chr_ctgry_3').val('<?= $record['ctgry_3'] ?>');
        });
    })
</script>
</head>
    ...
    <div class="wrap">
        ...
        <div class="body">
            <div class="container input_ctgry">
                ...
                <div class="select_wrap">
                    ...
                    <div class="input_container">
                        <div class="date_wrap pay input_con">
                            ...
                            <div class="input_date">
                                <div class="input_wrap">
                                    <input type="text" id="o_pay" inputmode="numeric" value="<?= htmlspecialchars($record['o_pay']) ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="date_wrap pay input_con">
                            ...
                            <div class="input_date">
                                <div class="input_wrap">
                                    <input type="text" id="o_reward" inputmode="numeric" value="<?= htmlspecialchars($record['o_reward'] ?? '') ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="input_con">
                            ...
                            <div class="input_text flex">
                                <div class="input_wrap">
                                    <input type="text" id="o_amount" class="a_right" inputmode="numeric" pattern="\d*" value="<?= $record['o_amount'] ? number_format($record['o_amount']) : '' ?>">
                                </div>
                        ...
                        <div class="input_con">
                            ...
                            <div class="input_text flex">
                                <div class="input_wrap">
                                    <textarea name="" id="o_description" placeholder="해당 지출내역에 대한 상세한 내역을 남겨주세요. ex) 권빅뱅리더, 동태양조원 1대1 식사"><?= htmlspecialchars($record['o_description'] ?? '') ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <a href="index_multi_history.php" class="btn_cancel">취소</a>
